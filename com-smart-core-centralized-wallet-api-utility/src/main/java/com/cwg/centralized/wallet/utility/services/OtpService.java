...
import com.cwg.centralized.wallet.utility.domain.Otp;
...
import com.cwg.centralized.wallet.utility.repo.OtpRepository;
import com.cwg.centralized.wallet.utility.repo.UserDetailsRepo;
import com.cwg.centralized.wallet.utility.util.StrongAES;
...
public class OtpService {
    ...
    @Value("${gen.otp.expiry-period-in-minutes}")
    ...
    public BaseResponse createAndSendOtpEmail(OtpRequest request) {
        ...
        try {
            ...
            if (request.getOtp() == null) {
                otpValue = !environment.equals("local") ? generateOtp() : 123456;
            } else {
                isOtpCreatedFromExternalService = true;
                otpValue = !environment.equals("local") ? Integer.valueOf(request.getOtp()) : 123456;
            }

            List<UserDetails> getRecordDevice = userDetailsRepo.findByUserEmailIdDe(request.getEmailAddress());
            String name = null;
            if (getRecordDevice.size() > 0) {

                name = getRecordDevice.get(0).getProductName();
            }
            ...
            req.setSubject("SmartCore Pay Wallet");
    ...
    private String generateOtpMsg(int otp, String name
        ...
        String msg = "<!DOCTYPE html>\n"
                ...
                + "    <div class=\"container\">\n"
                + "    <img src=\"/assets/demo-email-image.png\" alt=\"smartcorelogo\" />\n"
                + "    <h2>Get started with Wallet Service</h2>\n"
                + "    <img src=\"/assets/demo-email-image.png\" alt=\"shield\" />\n"
                + "    <div>\n"
                + "        <p>Hello " + name + "</p>\n"
                + "\n"
                + "        <p>DO NOT DISCLOSE</p>\n"
                ...
                + "        <hr class=\"line\" />\n"
                + "        <div class=\"footer\">\n"
                + "            <p>If you run into issues while using Wallet Service, can get our real-time chat support via WhatsApp or even\n"
                + "                write to us at:\n"
                + "            </p>\n"
                + "\n"
                + "            <p style=\"color:#69add8\">Wallet Service</p>\n"
                + "        </div>\n"
                + "\n"
                + "    </div>\n"
                + "</div>\n"
        ...
        String otpMessage = "Do not share with anyone including Wallet Service Support staff. " + "Your Wallet Service code: "
                + String.valueOf(otp) + "." + " Call {0000000000} now if you didn't initiate this request.";
        return otpMessage;
    }
    ...
    public BaseResponse createAndSendOtpSMSOnly(OtpRequest request) {
        ...
        try {
            ...
            if (!isOtpCreatedFromExternalService) {
                if (!environment.equals("local")) {
                    System.out.println(":::::::: createAndSendOtpSMSOnly" + "::::: ");
